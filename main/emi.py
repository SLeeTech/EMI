...
class emi:
    ...
    def encrypt_ske(message, secret_key):
        ## transfroming the secret_key into Base64 Key
        h = blake2b(digest_size=16)
        h_pw = h.update(bytes(secret_key.encode('utf-8')))
        hh = h.hexdigest()
    ...
    # ske = symmetric key encryption
    def decrypt_ske(encrypted_message, secret_key):
        h = blake2b(digest_size=16)
        h_pw = h.update(bytes(secret_key.encode('utf-8')))
        hh = h.hexdigest()
        pw_string = str(hh).encode('utf-8')
    ...
    def extract_json(decrypt_message):
        ...
        json_string = decrypt_message.decode("ascii")
